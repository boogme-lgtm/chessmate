...
- [x] S49-2: Board too narrow — chessboard squeezed by flex layout; needs to be square and fill available width
...
- [x] S49-25: Board still too small — flex-1 left column + min(calc(100%-18px), calc(90vh-8rem)) board wrapper still resolves too small; root cause is self-start collapsing the left column to zero height so 100% inside it is 0; fix: remove self-start, give the board wrapper an explicit pixel size using a CSS container query or a simpler approach: make the main area a fixed-height flex row and let the board wrapper use aspect-ratio:1 with height:100%
- [x] S49-26: NAG $11 shows raw text — NAG 11 is not in the NAG_GLYPHS map; it should map to "⩲" (White is slightly better) or display nothing rather than the raw "$11" token; add all missing common NAG codes
- [x] S49-27: Last-move highlight square doesn't follow keyboard navigation — react-chessboard v5 has a lastMove prop (or lastMoveSquares) that should be driven by the from/to squares of the current move; currently the highlight stays on the last mouse-touched move instead of the current board position

## Sprint 49 Fix-9 — Last-Move Frame + Branch Choice Popup (S49-28, S49-29)

- [x] S49-28: White click-frame persists on wrong square — react-chessboard v5 renders its own internal click/focus highlight that is NOT controlled by squareStyles; it is a built-in CSS :focus or internal state ring; must suppress it by adding onSquareClick handler that clears internal selection, OR use clearArrowsOnClick, OR override via squareStyle with outline:none; also the squareStyles orange tint may not be visible because the internal white frame is drawn on top — need to verify squareStyles z-index vs internal frame
- [x] S49-29: No branch choice popup when right-arrowing into a bifurcation — when the user presses right-arrow and the current main-line node has variations, show a small inline popup (Popover or DropdownMenu) listing all candidate moves (main line + sideline first moves) so the user can pick which branch to follow; after choosing, navigate to that move; if user presses right-arrow again without choosing, default to main line

## Sprint 49 Fix-10 — Branch Picker Keyboard Nav + PGN Auto-Scroll (S49-30, S49-31)

- [x] S49-30: Branch picker not keyboard-navigable — ↑/↓ arrows currently jump to start/end of main line instead of cycling through the picker options; need a focusedChoiceIndex state (0 = first option highlighted by default when picker opens); ↑/↓ cycle through branchChoices; Enter/Space confirms the focused choice; Escape still defaults to main line; the highlighted row should show bg-muted/60 and the terracotta color for the focused item
- [x] S49-31: PGN viewer does not scroll to follow the current move — the active move (bold terracotta) can be anywhere in the move list but the scroll position never changes; need a ref on the active move button element and a useEffect that calls scrollIntoView({block:'nearest',behavior:'smooth'}) whenever displayFen changes; the scroll target is the active button inside the .overflow-y-auto move list div; use the same data-radix-scroll-area-viewport query pattern as AIChatBox.tsx if wrapped in ScrollArea, or directly on the parent div if not
...
## Sprint 50 — PGN Analysis Mode + Send-to-Coach Recap

- [x] S50-1: DB schema — add `pgnAnalyses` table (id, lessonId nullable, contentItemId nullable, studentId, coachId nullable, originalPgn mediumtext, annotatedPgn mediumtext, title varchar, status enum(draft/sent), sentAt timestamp, createdAt, updatedAt) — supports both lesson-linked and standalone analysis sessions
- [x] S50-2: DB migration — run pnpm db:push after schema change
- [x] S50-3: Server router — add `analysis` tRPC router with procedures: createAnalysis(lessonId?, contentItemId?, originalPgn, title), saveAnnotatedPgn(id, annotatedPgn), sendToCoach(id), getMyAnalyses(), getAnalysisById(id)
- [x] S50-4: PgnViewerModal — add "Analyse" toggle button in the nav bar; when active, switches to analysis mode: board becomes interactive (draggable pieces), moves played on the board are recorded as a new sideline from the current position, the PGN text panel shows the growing annotation tree in real time
- [x] S50-5: Analysis mode — annotation editor: clicking a move in the text panel (or after playing a move) shows a small inline text input for adding a comment; pressing Enter or clicking away saves the comment to the node; comments render as italic text in the PGN panel (already supported by the existing comment rendering)
- [x] S50-6: Analysis mode — NAG annotation toolbar: a row of clickable glyph buttons (!, ?, !!, ??, !?, ?!) appears below the board when a move is selected; clicking a glyph toggles it on/off on the selected node; the PGN text panel updates immediately
- [x] S50-7: Analysis mode — PGN serializer: a serializePgn(nodes, headers) function that produces a valid PGN string from the current in-memory tree (main line + all variations, comments in {curly braces}, NAGs as $N tokens, headers in [Key "Value"] format); this is the inverse of parsePgnTree
- [x] S50-8: Analysis mode — save button: calls saveAnnotatedPgn tRPC mutation with the serialized PGN; shows a toast on success; the analysis record is created on first save if it doesn't exist yet
- [x] S50-9: Send-to-coach recap: a "Send to Coach" button (only visible when the viewer was opened from a lesson context with a coachId); clicking opens a small dialog with a text area for a personal note + preview of the annotated PGN; confirming calls sendToCoach mutation which: (a) saves the final annotated PGN, (b) creates a message in the lesson's chat with contentType='pgn' and the serialized PGN as content, (c) sends an in-app notification to the coach; shows success toast
- [x] S50-10: PgnViewerModal — accept optional props: lessonId, coachId, analysisId (for reopening a saved analysis); when analysisId is provided, load the annotatedPgn from the DB instead of the original PGN

## Sprint 50 Fix-1 — Coach Analysis Support

- [x] S50-F1: Coach blocked from saving analysis — analysisRouter.create checks `lesson.studentId !== ctx.user.id` and throws FORBIDDEN for coaches; db helpers `updatePgnAnalysis`, `getPgnAnalysisById`, `listPgnAnalysesByStudent` all filter by `studentId` only; fix: (a) add `viewerRole: "student" | "coach"` prop to PgnViewerModal; (b) in `create`, allow the lesson's coachId as well — if caller is the coach, set `studentId` to the lesson's studentId and `coachId` to ctx.user.id; (c) add `getPgnAnalysisByIdForCoach(id, coachId)` and `updatePgnAnalysisForCoach(id, coachId, pgn)` db helpers that filter by `coachId` instead; (d) router `save` and `byId` try student ownership first, then coach ownership; (e) `sendToCoach` similarly allows the coach to send (in which case it sends to the student instead)
- [x] S50-F2: "Send to Coach" label wrong for coaches — when `viewerRole === "coach"`, the button should read "Send to Student" and the mutation should post the message to the lesson chat and email the student instead of the coach

## Review Schema Fix

- [x] S-REV-1: Reviews INSERT fails for both coach and student — the live DB `reviews` table was created by migration 0001 with `studentId INT NOT NULL` and `coachId INT NOT NULL` columns; the current `drizzle/schema.ts` was later changed to use `reviewerId`, `revieweeId`, `reviewerType` columns that don't match the live DB; Drizzle generates an INSERT with the new column names which the DB rejects; fix strategy: (1) revert `drizzle/schema.ts` reviews table back to the original `studentId`/`coachId` design so it matches the live DB exactly, (2) update `createReview` db helper to insert `studentId` and `coachId` directly (derive from `reviewerType` in the caller), (3) update `review.submit` router procedure to pass `studentId` and `coachId` instead of `reviewerId`/`revieweeId`/`reviewerType`, (4) update `getCounterpartReview`, `getReviewsByCoach`, `setReviewsVisibleForLesson`, and `getReviewByLessonAndReviewer` db helpers to use `studentId`/`coachId` columns, (5) run `pnpm db:push` to confirm zero drift; do NOT attempt to rename or drop the old columns as data may exist; the `reviewerType` concept is implicit: if the reviewer is the lesson's student → `reviewerType = 'student'`, if the reviewer is the lesson's coach → `reviewerType = 'coach'` — this can be derived at query time from the lesson row without storing it

- [ ] S-STOREFRONT-3-1: Add "new_content_sale" to notifications.type enum + pnpm db:push
- [ ] S-STOREFRONT-3-2: Add getCoachContentEarnings() helper in server/db.ts
- [ ] S-STOREFRONT-3-3: Update getCoachEarningsSummary to include contentEarningsCents
- [ ] S-STOREFRONT-3-4: Post-purchase coach in-app notification in confirmStorefrontPurchase
- [ ] S-STOREFRONT-3-5: Post-purchase student receipt email in confirmStorefrontPurchase
