...
- [ ] S49-30: Branch picker not keyboard-navigable — ↑/↓ arrows currently jump to start/end of main line instead of cycling through the picker options; need a focusedChoiceIndex state (0 = first option highlighted by default when picker opens); ↑/↓ cycle through branchChoices; Enter/Space confirms the focused choice; Escape still defaults to main line; the highlighted row should show bg-muted/60 and the terracotta color for the focused item
- [ ] S49-31: PGN viewer does not scroll to follow the current move — the active move (bold terracotta) can be anywhere in the move list but the scroll position never changes; need a ref on the active move button element and a useEffect that calls scrollIntoView({block:'nearest',behavior:'smooth'}) whenever displayFen changes; the scroll target is the active button inside the .overflow-y-auto move list div; use the same data-radix-scroll-area-viewport query pattern as AIChatBox.tsx if wrapped in ScrollArea, or directly on the parent div if not
