...
- [ ] S50-1: DB schema — add `pgnAnalyses` table (id, lessonId nullable, contentItemId nullable, studentId, coachId nullable, originalPgn mediumtext, annotatedPgn mediumtext, title varchar, status enum(draft/sent), sentAt timestamp, createdAt, updatedAt) — supports both lesson-linked and standalone analysis sessions
- [ ] S50-2: DB migration — run pnpm db:push after schema change
- [ ] S50-3: Server router — add `analysis` tRPC router with procedures: createAnalysis(lessonId?, contentItemId?, originalPgn, title), saveAnnotatedPgn(id, annotatedPgn), sendToCoach(id), getMyAnalyses(), getAnalysisById(id)
- [ ] S50-4: PgnViewerModal — add "Analyse" toggle button in the nav bar; when active, switches to analysis mode: board becomes interactive (draggable pieces), moves played on the board are recorded as a new sideline from the current position, the PGN text panel shows the growing annotation tree in real time
- [ ] S50-5: Analysis mode — annotation editor: clicking a move in the text panel (or after playing a move) shows a small inline text input for adding a comment; pressing Enter or clicking away saves the comment to the node; comments render as italic text in the PGN panel (already supported by the existing comment rendering)
- [ ] S50-6: Analysis mode — NAG annotation toolbar: a row of clickable glyph buttons (!, ?, !!, ??, !?, ?!) appears below the board when a move is selected; clicking a glyph toggles it on/off on the selected node; the PGN text panel updates immediately
- [ ] S50-7: Analysis mode — PGN serializer: a serializePgn(nodes, headers) function that produces a valid PGN string from the current in-memory tree (main line + all variations, comments in {curly braces}, NAGs as $N tokens, headers in [Key "Value"] format); this is the inverse of parsePgnTree
- [ ] S50-8: Analysis mode — save button: calls saveAnnotatedPgn tRPC mutation with the serialized PGN; shows a toast on success; the analysis record is created on first save if it doesn't exist yet
- [ ] S50-9: Send-to-coach recap: a "Send to Coach" button (only visible when the viewer was opened from a lesson context with a coachId); clicking opens a small dialog with a text area for a personal note + preview of the annotated PGN; confirming calls sendToCoach mutation which: (a) saves the final annotated PGN, (b) creates a message in the lesson's chat with contentType='pgn' and the serialized PGN as content, (c) sends an in-app notification to the coach; shows success toast
- [ ] S50-10: PgnViewerModal — accept optional props: lessonId, coachId, analysisId (for reopening a saved analysis); when analysisId is provided, load the annotatedPgn from the DB instead of the original PGN
